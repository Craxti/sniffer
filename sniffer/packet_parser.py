...
def parse_packet(data):
    ...
    if ethernet_type == 0x0800:  # IPv4
        ...
        if protocol == 6:  # TCP
            return parse_tcp_header(data[14 + ip_header[1]:])

        elif protocol == 17:  # UDP
            return parse_udp_header(data[14 + ip_header[1]:])

        elif protocol == 1:  # ICMP
            return parse_icmp_header(data[14 + ip_header[1]:])

    elif ethernet_type == 0x86DD:  # IPv6
        ipv6_header = parse_ipv6_header(data[14:])
        next_header = ipv6_header[4]

        if next_header == 6:  # TCP
            return parse_tcp_header(data[54:])

        elif next_header == 17:  # UDP
            return parse_udp_header(data[54:])
